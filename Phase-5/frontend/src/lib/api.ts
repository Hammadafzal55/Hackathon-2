...
import type { ReminderRead } from '../types/task';
...
class ApiClient {
  ...
  /**
   * Fetch tasks for the authenticated user with optional search/filter/sort params
   */
    ...
    return this.makeRequestWithRetry(async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          return { tasks: data, total_count: data.length };
        }
        return { tasks: data.tasks || [], total_count: data.total_count || 0 };
      } catch (error) {
        clearTimeout(timeoutId);
        if (error instanceof Error && error.name === 'AbortError') throw new Error('API request timed out');
        throw error;
      }
    });
  }
   ...
   */
  getNotifications = async (unread?: boolean, limit = 20, offset = 0): Promise<{ notifications: NotificationRead[]; total_count: number; unread_count: number }> => {
    const headers = await this.getHeaders();
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/notifications?${qs}`, { method: 'GET', headers, signal: controller.signal });
      clearTimeout(timeoutId);
      if (!response.ok) throw new ApiError(`API Error: ${response.status}`, response.status, response);
  ...
  markNotificationRead = async (id: string): Promise<void> => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      const response = await fetch(`${this.baseUrl}/api/notifications/${id}/read`, { method: 'PATCH', headers, signal: controller.signal });
      clearTimeout(timeoutId);
      if (!response.ok) throw new ApiError(`API Error: ${response.status}`, response.status, response);
    } catch (error) {
      clearTimeout(timeoutId);
  ...
  markAllNotificationsRead = async (): Promise<void> => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      const response = await fetch(`${this.baseUrl}/api/notifications/mark-all-read`, { method: 'POST', headers, signal: controller.signal });
      clearTimeout(timeoutId);
      if (!response.ok) throw new ApiError(`API Error: ${response.status}`, response.status, response);
    } catch (error) {
      clearTimeout(timeoutId);
  ...
  /**
   * Get all distinct tags for the authenticated user
   */
  getTags = async (): Promise<string[]> => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      const response = await fetch(`${this.baseUrl}/api/tags`, { method: 'GET', headers, signal: controller.signal });
      clearTimeout(timeoutId);
      if (!response.ok) throw new ApiError(`API Error: ${response.status}`, response.status, response);
      const data = await response.json();
      return data.tags || [];
   ...
   */
  getTaskReminders = async (taskId: string): Promise<ReminderRead[]> => {
    const headers = await this.getHeaders();
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      const response = await fetch(`${this.baseUrl}/api/tasks/${taskId}/reminders`, { method: 'GET', headers, signal: controller.signal });
      clearTimeout(timeoutId);
      if (!response.ok) throw new ApiError(`API Error: ${response.status}`, response.status, response);
      return await response.json();
    } catch (error) {
